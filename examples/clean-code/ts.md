...
TypeScript ❌ BAD / ✅ GOOD examples for the rules in [`CLEAN-CODE.md`](../../CLEAN-CODE.md).
Each section is keyed by rule number. Read the rule text and one-line summary in
`CLEAN-CODE.md`; the code below shows the violation and the fix.
...
## Rule 2 — Use descriptive, unambiguous names
...
## Rule 4 — Use searchable names — avoid magic numbers
...
## Rule 5 — Avoid encodings and type noise
...
## Rule 6 — Be consistent — same pattern everywhere
...
## Rule 7 — Functions should be small and do ONE thing
...
## Rule 8 — One level of abstraction per function
...
## Rule 10 — Prefer fewer arguments — use parameter objects when needed
...
## Rule 11 — Don't use flag arguments — split into separate functions
...
## Rule 13 — Explain yourself in code, not in comments
...
## Rule 14 — Use comments to explain INTENT, WARNINGS, and non-obvious decisions
...
## Rule 15 — Never comment out dead code — delete it
...
## Rule 16 — Declare variables close to their usage
...
## Rule 17 — Place caller above callee — high-level first
...
## Rule 18 — Hide internal structure — expose behavior, not data
...
## Rule 19 — Follow the Law of Demeter — talk only to direct neighbors
...
## Rule 20 — Always find the root cause — don't suppress errors silently
...
## Rule 21 — One logical assertion per test
...
## Rule 22 — Fast — tests should run in milliseconds; no real I/O
...
## Rule 23 — Independent — tests must not depend on each other
...
## Rule 24 — Readable — test names describe behavior, not implementation
...
## Rule 25 — Single Responsibility — classes should have one reason to change
...
## Rule 26 — Use dependency injection — don't instantiate collaborators inside classes
...
## Rule 27 — Separate multi-threading / async code from business logic
...
## Rule 28 — Keep it simple (KISS) — reduce complexity
...
## Rule 29 — Prefer value objects over primitives
...
## Rule 30 — Rigidity — don't make changes that cascade everywhere
...
## Rule 31 — Opacity — deeply nested code is hard to read; flatten it
...
## Rule 32 — Needless complexity — don't over-engineer
...
## Rule 33 — Prefer discriminated unions over boolean flags
...
## Rule 34 — Use `unknown` over `any` — force explicit narrowing
